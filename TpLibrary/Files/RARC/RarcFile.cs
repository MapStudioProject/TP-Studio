                ...
                for (int dir = 0; dir < DirectoryCount; dir++)
                {
                    for (int n = 0; n < Directories[dir].NodeCount; n++)
                    {
                        ...
                        else
                        {
                            ...
                            //entry.FileData = new SubStream(reader.BaseStream, dataPos, entry.Size);
                            entry.SetData(reader.getSection((int)dataPos, (int)entry.Size));
            ...
            for (int dir = 0; dir < _savedNodes.Count; dir++)
            {
                for (int n = 0; n < _savedNodes[dir].Children.Count; n++)
                {
                    ...
                    else
                    {
                        ...
                        _savedFileData.Add(fileEntry.AsBytes());
                        writer.Write(0);
                    }
                }
